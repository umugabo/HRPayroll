...
<body>
	...
	<div style="border:0px solid black;height:auto;margin-left:-1px;margin-top:-3px;width:1350px;margin-top:-3px;background-image:url(../photos/background.jpg);">
	<div style="border:0px solid black;height:160px;width:1350px;background-image:url(../photos/banner3.jpg);margin-top:0px">
</div>

<div class="container"style="margin:-3px;padding:2px;margin-top:-1px;width:190px;position:absolute">
<nav>
		<ul class="menu">
			<li>
...
<?php
echo '<img src="data:image/jpeg;base64,'.base64_encode( $userRow['image'] ).'"style="width:20px;height:20px;border-radius:50%;margin-left:300px;height: 80px;margin-left: 290px;margin-top: -7px;width:80px;float:right;"/>';?></a></div>
	</div>
<A HREF="download.php">
<img src="../photos/google.png"width="153"height="78"style="float:right;position:fixed;margin-top:30%;margin-left:39%"></A>
<div style="border:0px solid blue;height:auto;width:1158px;margin-left:190px;margin-top:0px;background: #FFF;border-radius:15px;">
...
<table width="1157" height="209" border="1">
  ...
  $con=mysql_connect('localhost','root','');
mysql_select_db("dbtest",$con);
$sql=mysql_query("select * from sectorquestion where usec='matyazo' order by id DESC");
$i=1;
while (
$row=mysql_fetch_array($sql)){
  echo "<tr>
    ...
    <td>".$row['lfn']."&nbsp;".$row['lln']."<br>".$row['post']."<br>".$row['depart']."<br><b>on&nbsp&nbsp".$row['time']."</b></td>
