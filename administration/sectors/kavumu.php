...
<body>
	...
	<div style="border:0px solid black;height:auto;width:1350px;margin-top:-3px;background-image:url(../photos/background.jpg);">
	<div style="border:0px solid black;height:160px;width:1350px;background-image:url(../photos/banner3.jpg);margin-top:0px">
	</div>

<div class="container"style="margin:-3px;padding:2px;margin-top:-2px;width:180px;position:absolute">
<nav>
	<ul class="menu">
			<li>
...
<div style="border:0px solid blue;height:auto;width:1160px;padding-right:11px;padding-left:-5px;margin-left:180px;margin-top:0px;background: #FFF;border-radius:15px;">
...
<table width="1157" height="209" border="1">
  ...
  $con=mysql_connect('localhost','root','');
mysql_select_db("dbtest",$con);
$sql=mysql_query("select * from sectorquestion where usec='kavumu' order by id DESC");
$i=1;
while (
$row=mysql_fetch_array($sql)){
  echo "<tr>
    <td height='109'>".$i."</td>
    <td style:'padding:3px;width:140px'font-size:16px>".strtoupper($row['ufn'])."<br>".$row['uln']."<br>SECTOR:".strtolower($row['usec'])."<br>CELL:".strtolower($row['ucell'])."<br>ID:".$row['uid']."</td>
    ...
    <td>".strtoupper($row['lfn'])."&nbsp".$row['lln']."<br>".$row['post']."<br>".$row['depart']."<br><b>on&nbsp&nbsp".$row['date']."</b></td>
